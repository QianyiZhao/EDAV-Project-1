...
```{r qplot, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
###################################################################
#Group Name:
#Project 1
#Due: 2/11/2016
...
library(ggplot2)
library(plyr)
raw_response <- as.data.frame(read.table("C:/Users/tbenham002/Documents/Personal/School/Survey_Response.csv", header=TRUE, fill = TRUE, sep=","))
...
```
> In this section we explore the distribution of student R experience

```{r Class_dist, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
...
```
> The experience distribution is rather normally distributed however there seems to a minority cluster of under experienced people. So next step is to understand who that group is.

```{r prog_split, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
...
```
> We can see here there is a cluster of Data Science certificate students with below average levels of R experience.
We should affirm that the number of DS Cert students is proportionally significant as an additional check.

```{r prog_count, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
table(agg_response$Program)
```

```{r dist_gender, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
...
```
> We can see the number of DS certificate students is significant.
We also looked at gender and wile she/her is bi-modal, it does not look like this is a driver of the under performing group.
> Below we show the distribution by program on a single density graph and the distribution difference is very clearly visible.

```{r dist_prog_inone, fig.width=8, fig.height=6, message=FALSE, echo=FALSE}
